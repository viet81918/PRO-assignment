...
public class CustomerManagement extends Menu<String> {
    ...
    private void cusBuyMenu() throws ParseException {
        String search[] = { "Hien thi tat ca sach ", "Tim kiem sach ", " Them sach vao gio hang ",
                " Hien thi nhung review da viet", " Hien thi Muc giam gia ", " Viet review sau khi nhap ID sach" };
        Menu m = new Menu("Buy Menu", search) {
            ...
            public void execute(int n) throws ParseException {
                switch (n) {
                    ...
                    case 5: {
                        // checkdis();
                        System.out.println("Chuc nang dang bao t ri , xin quy khach cho doi sau");
                        break;
                    }
                    case 6: {
                        String idC = "";
                        do {
                            System.out.print("Enter Customer ID: ");
                            idC = scanner.nextLine().trim();
                        } while (!val.checkIDcus(idC));
                        String idB = getValue("NHap Id sach muon viet review");
                        Writereview(idC, idB);
                        break;
                    }
                    default:
    ...
    private void cusRentMenu() throws ParseException {
        String[] search = { "Hien thi tat ca sach ", "Tim kiem sach ", " Them sach vao gio hang ",
                " Hien thi nhung review da viet", " Viet review sau khi nhap ID sach", "Nhap thoi gian tra sach" };
        Menu m = new Menu("Rent Menu", search) {
            ...
            public void execute(int n) throws ParseException {
                switch (n) {
                    ...
                    case 6: {
                        String ID = getValue("Nhap ID sach thue ");
                        setDayReturn(ID);
                    }
                    default:
                    System.out.println("Out of function!");
    ...
    private void addRentbag() {
        ...
        int num = val.getValidIntegerInput();
        for (int i = 0; i < num; i++) {
            String id = getValue("Nhap id sach muon thue :");
            Bags.add(id);
        }
        for (String ID : Bags) {
            System.out.println(admin.searchRentBook1(p -> ((RentBook) p).getBookID().equalsIgnoreCase(ID)));
        }
    }
    ...
    private void SearchBuyPrice() throws ParseException {
        String search[] = { "Nhap khoang gia mua muon tim", " Nhap gia mua toi thieu muon tim ",
                " Nhap gia mua toi da muon tim " };
    ...
    private void setDayReturn(String iD) {
        for (Object rb : admin.searchRentBook1(p -> ((RentBook) p).getBookID().equalsIgnoreCase(iD))) {
            String Dayrent = val.inputDay();
            String Dayreturn = val.inputDay();
            cm.setDayRentBook((RentBook) rb, Dayrent, Dayreturn);
        }
    }

    private void AddBuyintoBag() throws FileNotFoundException, ParseException {
        ArrayList<String> Bags = new ArrayList<>();
        System.out.println(" Nhap so sach muon mua ");
        int num = val.getValidIntegerInput();
        for (int i = 0; i < num; i++) {
            String id = getValue("Nhap id sach muon mua :");
            Bags.add(id);
        }
        for (String ID : Bags) {
            System.out.println(admin.searchBuyBook1(p -> p.getBookID().equalsIgnoreCase(ID)));
        }
    }
